...
import { garageDoors, getProductBySlug } from "@/lib/products-data"
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const product = getProductBySlug("garage-doors", slug)
...
export function generateStaticParams() {
  return garageDoors.map((product) => ({
    slug: product.slug,
  }))
}

export default async function GarageDoorProductPage({ params }: PageProps) {
  const { slug } = await params
  const product = getProductBySlug("garage-doors", slug)
  ...
  const otherProducts = garageDoors.filter((p) => p.slug !== slug).slice(0, 2)
    ...
    <>
      ...
      <main>
        <PageHeader
          ...
          breadcrumbs={[
            { name: "Garage Doors", href: "/garage-doors" },
            { name: product.name, href: `/garage-doors/${product.slug}` },
          ]}
        />
        ...
        <section className="py-12 lg:py-20 bg-muted">
          <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
            <h2 className="text-2xl font-bold text-foreground font-serif mb-8">Explore Other Garage Doors</h2>
                  ...
                  <div className="flex flex-col sm:flex-row">
                    ...
                    <CardContent className="p-6 flex flex-col justify-center">
                      ...
                      <Button asChild variant="outline" size="sm" className="w-fit bg-transparent">
                        <Link href={`/garage-doors/${p.slug}`}>Learn More</Link>
                      </Button>
                    </CardContent>
                  </div>
                </Card>
